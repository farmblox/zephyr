...
#define SX1261_TX_PWR_MAX 5 // should be +22
...

{
	...
	// There's something about errata that should go here
...
void lbm_driver_antenna_configure(const struct device *dev, enum lbm_modem_mode mode)
{
	...
	// TODO: No idea what to actually do here!
...
// static void sx126x_dio1_callback(const struct device *dev, struct gpio_callback *cb, uint32_t
// pins)
...
static int sx126x_init(const struct device *dev)
{
	...
	LOG_ERR("INIT INIT INIT INIT INIT INIT INIT!!");
...
#define SX1261_DEFINE(node_id) SX126X_DEFINE(node_id)

DT_FOREACH_STATUS_OKAY(semtech_sx1261, SX1261_DEFINE);
*/

static const struct lbm_sx126x_config config = {
	.lbm_common.ralf = RALF_SX126X_INSTANTIATE(DEVICE_DT_INST_GET(0)),
	.spi = SPI_DT_SPEC_INST_GET(0, SPI_WORD_SET(8) | SPI_OP_MODE_MASTER | SPI_TRANSFER_MSB, 0),
...
static struct lbm_sx126x_data data;

DEVICE_DT_INST_DEFINE(0, sx126x_init, NULL, &data, &config, POST_KERNEL, CONFIG_LORA_INIT_PRIORITY,
		      &lbm_lora_api);
